// ====================================================================
//  TTRL SIGNUP BOT - FULL UPDATED VERSION (2025-12-14)
// ====================================================================
...
client.on(Events.InteractionCreate, async (interaction) => {
  // Debug logger – helps confirm we receive events
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      // ===============================================================
      // /ttrl-tt-panel (admin posts TT info + button)
      // ===============================================================
      if (interaction.commandName === "ttrl-tt-panel") {
        if (!interaction.inGuild()) {
          return interaction.reply({ content: "Use this in a server.", ephemeral: true });
        }
        ...
        const embed = new EmbedBuilder()
          .setTitle("TTRL Time Trials")
          .setDescription([
            "Time Trials are **still to be determined**.",
            "",
            "When you are ready, click the button below to submit your lap times for:",
            "- Bahrain",
            "- Austria",
            "- Silverstone",
            "",
            "Format: `mm:ss.mmm` (example: `01:28.432`)."
          ].join("\n"))
          .setColor(0x00aeff);

        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("ttrltt|open")
            .setLabel("Submit Time Trial Times")
            .setStyle(ButtonStyle.Primary),
        );

        await interaction.channel.send({ embeds: [embed], components: [row] });
        return interaction.reply({ content: "Time Trial panel posted.", ephemeral: true });
      }

      // ===============================================================
      // /ttrl-timetrial (drivers submit times; multiple allowed)
      // ===============================================================
    ...
    // Time Trial button from /ttrl-tt-panel
    if (interaction.customId === "ttrltt|open") {
      return interaction.reply({
        ephemeral: true,
        content: [
          "Time Trials are still being determined.",
          "",
          "Please submit your lap times using the `/ttrl-timetrial` command:",
          "- Tracks: Bahrain, Austria, Silverstone",
          "- Format: `mm:ss.mmm` (example: `01:28.432`)",
          "",
          "You can submit more than once; each submission will be logged as a separate entry."
        ].join("\n")
      });
    }
